...
if [ "$SELECT_PET" = "true" ] && codex_is_running; then
  fail "Close Codex before selecting the pet so config.toml cannot be overwritten while the app is saving it. Use --no-select to install files only."
fi
...
MANIFEST_TEMP="$(/usr/bin/mktemp "$TARGET_DIR/.pet.json.XXXXXX")"
SPRITESHEET_TEMP="$(/usr/bin/mktemp "$TARGET_DIR/.spritesheet.webp.XXXXXX")"
cleanup() {
...
# pet.json is the commit marker: Codex never observes a manifest that points
# at a not-yet-published spritesheet.
