...
# 4) Or configure the bundled Miku pastel preset without restarting Codex
~/.codex/codex-dream-skin-studio/scripts/configure-miku-preset-macos.sh --no-apply
...
# 6) Optional: menu bar (SwiftBar) — apply / pause / change image
...
`Configure Miku Theme.command` saves the bundled `miku-pastel` preset without
restarting Codex. The preset was reconstructed from a visual mockup as a real
interactive skin:

- the bundled image contains only the character and atmosphere, with no fake
  sidebar, cards, text, buttons, or composer baked into the bitmap
- the home route uses the image as a large native hero behind live Codex
  heading, suggestion cards, project selector, and composer
- task routes use a much stronger white veil so messages and code stay readable
- the sidebar, Outputs/Sources panel, code blocks, user messages, and composer
  use cyan/pink frosted surfaces matched to Codex `26.707.91948`
- older home selectors remain in place, while the current task route uses
  `.app-shell-main-content-*` and `.thread-scroll-container`

Configure only:

```bash
./scripts/configure-miku-preset-macos.sh --no-apply
```

Configure and immediately start/reapply:

```bash
./scripts/configure-miku-preset-macos.sh --apply
```
